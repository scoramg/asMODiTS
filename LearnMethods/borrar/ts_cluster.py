...
class ts_kmeans(object):
	...
	def fit(self,data,num_iter,w,progress=False):
		'''
		...
		'''
		self.centroids=random.sample(data,self.rbfnn_k)
		...
		for n in range(num_iter):
			...
			for ind,i in enumerate(data):
				...
				for c_ind,j in enumerate(self.centroids):
					if self.options.dist_metric == 'dtw':
						dist_measure = DTW(options=self.options)
					cur_dist=dist_measure.distance(i,j)
