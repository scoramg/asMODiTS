...
def _cost_matrix_region(x, y, dist, region):
    ...
    for i in prange(n_timestamps_1):
        ...
        cost_mat[i, region[0, i]:region[1, i]] = dist(x[i], y[region[0, i]:region[1, i]])
    return cost_mat
...
def _project_cost_matrix_region(cost_mat, region):
    ...
    for i in prange(n_timestamps_1):
        ...
        cost_mat_projected[i, region[0, i]:region[1, i]] = cost_mat[i, region[0, i]:region[1, i]]
    return cost_mat_projected


@njit() #le quite el prange
...
def _check_input_dtw(x, y, precomputed_cost, dist, method):
    ...
    if dist == "precomputed":
        #cost = check_array(precomputed_cost, ensure_min_samples=2, ensure_min_features=2, ensure_2d=True,force_all_finite=False, dtype='float64')
...
def _input_to_cost(x, y, dist, precomputed_cost, region):
    ...
    #cost_mat = check_array(cost_mat, ensure_min_samples=2,ensure_min_features=2, ensure_2d=True,force_all_finite=False, dtype='float64')
    return cost_mat
...
def sakoe_chiba_band(n_timestamps_1, n_timestamps_2=None, window_size=0.1):
    ...
    scale, horizontal_shift, vertical_shift = _check_sakoe_chiba_params(n_timestamps_1, n_timestamps_2, window_size)
...
class DTW:
    ...
    def get_dtw_params(self):
        params = {}
        #print(self.options.dtw_method)
        if self.options.dtw_method == 'classic':
             params = None
        if self.options.dtw_method == 'sakoechiba':
            params["window_size"] = self.options.dtw_sakoechiba_w
        if self.options.dtw_method == 'itakura':
            params["max_slope"] = self.options.dtw_itakura_mslope
        if self.options.dtw_method == 'region': 
            params["region"] = np.array(self.options.dtw_region_reg)
        if self.options.dtw_method == 'multiscale': 
            params["resolution"] = self.options.dtw_multiscale_res
            params["radius"] = self.options.dtw_multiscale_r
        if self.options.dtw_method == 'fast':
            params["radius"] = self.options.dtw_fast_r
        return params
         
    def distance(self, dataset1, dataset2):
        """
        ...
        opts = self.get_dtw_params() 

        #for i in range(n1):
        #    for j in range(n2):
        for i,j in list(itertools.product(np.arange(0,n1),np.arange(0,n2))):
                dist[i][j] = self._distance(dataset1[i], dataset2[j], opts)
        return dist
    
    #@jit(nopython=True, cache=True)
    def _distance(self, serie1, serie2, opts=None):
        if self.options.exec_type == 'cpu':
            #return self._dtw_cpu(x=serie1, y=serie2, options=opts)
            return dtw_distance.distance_fast(s1=np.array(serie1),s2=np.array(serie2),window=self.options.dtw_sakoechiba_w,use_pruning=True)
    
    def _dtw_cpu(self, x=None, y=None, options=None):
        ...
        **Adaptative constraint region**
        ...
        if options is None:
            options = dict()
        precomputed_cost = self.options.dtw_precost
        x, y, precomputed_cost, n_timestamps_1, n_timestamps_2 = _check_input_dtw(x, y, precomputed_cost, self.options.dtw_dist, self.options.dtw_method)
        region = _compute_region(n_timestamps_1, n_timestamps_2, self.options.dtw_method, self.options.dtw_dist, x=x, y=y, **options)
        cost_mat = _input_to_cost(x, y, self.options.dtw_dist, precomputed_cost, region)
        acc_cost_mat = accumulated_cost_matrix(cost_mat, region=region)
        dtw_dist = acc_cost_mat[-1, -1]
        if self.options.dtw_dist == 'square':
            dtw_dist = sqrt(dtw_dist)

        res = _return_results(dtw_dist, cost_mat, acc_cost_mat,
                            self.options.dtw_return_cost, self.options.dtw_return_accum, self.options.dtw_return_path)
        return res
